...
$numberOfArticle = $requestArticle->rowCount();
//echo "$numberOfArticle <br/>";
$totalArticle = ceil($numberOfArticle/$articleParPage);
//echo "$totalArticle";
if(!empty($_GET['t']) AND $_GET['t'] > 0 AND $_GET['t'] <= $totalArticle) {
...
$req = $db->prepare("SELECT * FROM articles WHERE section  = ? ORDER BY article_id DESC LIMIT " . $start . "," . $articleParPage);
			...
			<!-- pagination system -->
			    <?php
			        for($i=1;$i<=$totalArticle;$i++) {
			        	$myLink = "index.php?section=" .$section;
			           if($i == $currentPage) { ?>
