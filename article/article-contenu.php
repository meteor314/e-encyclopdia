...
if(!empty($_GET['article_id']) AND $_GET['article_id'] > 0) {
    ...
    $info_like_button  = $like_query->fetch();
...
<body style="word-wrap: break-word; text-align: justify; font-family: arial;">
		...
		<?php 
			if(!empty($_SESSION['id'])) {
				if($_SESSION['id'] == $infoArticle['user_id'] || $_SESSION['id'] == 2) { 
					?>
		...
		<div class="content">
			...
			<?php if(!empty($_SESSION['id'])) { ?>
				...
				<h6>
					...
					<h4 id="heart-msg" style="text-align: center;color :gray;">+<?= $like_query->rowCount() ?></h4>
				</h6>
